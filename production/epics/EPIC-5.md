...
- 验收标准：
  - [x] 对话 GDD §3.1 两个示例 JSON 原样加载运行
  - [x] 选项必选其一（取消/移动键忽略——边缘 4）
  - [x] 对话中踩传送点不可能发生（边缘 2：非 IDLE 触发器一律忽略）
  - [x] 对话中途读档后状态机回 IDLE（边缘 5）
...
- 验收标准：
  - [x] `check_flag` 不存在于代码中（done 标准原文）
  - [x] 悬空引用在加载期报错拦截（边缘 1）
  - [x] 非数字 phase 键报错；映射选取规则单测通过
  - [x] 9 种动作全部实装（wait/play_sfx 允许空实现占位）
...
- 验收标准：
  - [x] phase 值随存档往返无损
  - [x] 6 个配额 NPC（菲奥拉/客栈老板/广场小孩/武器店老头/镇口守卫/神秘旅行者）可观察到阶段对话变化
...
- 验收标准：
  - [x] **剧情四拍全程只改 JSON 不改代码地跑通**（done 标准原文，占位文本即可）
  - [x] 3 个切换点时序正确
...
- 验收标准：
  - [x] Boss 战前/后事件完整走通，phase 2→3 在战后段内生效
  - [x] Boss 失败读档后事件从头可再触发、无中途态残留（探索边缘 3）
