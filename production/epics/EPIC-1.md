...
- 验收标准：
  - [x] 空场景在 1080p 全屏下整数缩放、16px 贴图锐利无模糊
  - [x] 项目设置截图留档（后续所有 Epic 的渲染基线）
  - [x] git 提交规范运转（一次 Story 一次或多次提交）
...
- 验收标准：
  - [x] 4 个单例注册生效，信号可 emit/connect
  - [x] 无超边界方法（GameData 不做 IO、EventBus 不存状态——自查对照 A3 表）
...
- 验收标准：
  - [x] 白盒图 A→B→A 切换正常，淡入淡出可见
  - [x] 传入非法 payload 时拒绝切换且有日志
...
- 验收标准：
  ...
  - [x] 无 subpixel 抖动（1080p 下逐帧观察）
  - [x] 角色原点在脚底，站墙后/树后被正确遮挡
...
- 验收标准：
  - [x] 主街环线 ~90s 可走通，密度抽查（NPC+调查+宝箱点位已预留）
  - [x] 五层结构齐备，y-sort 视觉正确
  - [x] 室内区相机限定，门可进可出（teleport 走 E4-S6 的正式时序，此处允许简版）
...
- 验收标准：
  - [x] 按 Z/E 触发对话，逐字显示、按键可跳/翻
  - [x] 对话期间玩家移动锁定
  - [x] 对话内容来自 `data/json/dialogues/`，非硬编码
